...
def list_note_categories(db: Session = Depends(get_db)):
    return db.query(NoteCategory).order_by(NoteCategory.sort_order).all()


@router.post("/categories", response_model=NoteCategoryResponse)
def create_note_category(data: NoteCategoryCreate, db: Session = Depends(get_db)):
...
def update_note_category(category_id: int, data: NoteCategoryUpdate, db: Session = Depends(get_db)):
    cat = db.query(NoteCategory).filter(NoteCategory.id == category_id).first()
    if not cat:
...
def delete_note_category(category_id: int, db: Session = Depends(get_db)):
    cat = db.query(NoteCategory).filter(NoteCategory.id == category_id).first()
    if not cat:
        raise HTTPException(404, "分类不存在")
    db.query(Note).filter(Note.category_id == category_id).update({Note.category_id: None})
    db.delete(cat)
    db.commit()
    return {"message": "已删除"}
...
def extract_skills(note_id: int, db: Session = Depends(get_db)):
    """AI 提取笔记中的技能/方法论"""
